...
def make_generative_lm(
    ...
    **kwargs,
    ...
    kwargs["torch_dtype"] = torch.bfloat16 if bf16 else torch.float16 if fp16 else torch.float32
    config = LlamaConfig.from_pretrained(model_name_or_path)
    config.use_lora = kwargs["use_lora"]
    config.lora_r = kwargs["lora_r"]
    config.lora_alpha = kwargs["lora_alpha"]
    config.lora_dropout = kwargs["lora_dropout"]
    del kwargs["use_lora"]
    del kwargs["lora_r"]
    del kwargs["lora_alpha"]
    del kwargs["lora_dropout"]
    return model_cls.from_pretrained(model_name_or_path, config=config, **kwargs)
...
def pack_best_of_n(module_outputs: list, select_idx: torch.Tensor):
    '''
        module_outputs: [{'key1' :  tensor of [bsz, seq_len], 'key2': tensor of [bsz, seq_len]}, ]
        select_idx: torch tensor
    '''
    bsz = select_idx.shape[0]
    if type(module_outputs[0]) is dict:
        for key in module_outputs[0]:
            for idx, sidx in zip(range(bsz), select_idx):
                module_outputs[0][key][idx] = module_outputs[sidx][key][idx]
    elif type(module_outputs[0]) is list:
        for idx, sidx in zip(range(bsz), select_idx):
            module_outputs[0][idx] = module_outputs[sidx][idx]
