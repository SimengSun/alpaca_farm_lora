...
@dataclasses.dataclass
class DataCollatorForNotStackableDataset(object):
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, Tensor]:
        ret = {}
        for key in instances[0].keys():
            if type(instances[0][key]) is Tensor:
                ret[key] = torch.stack([instance[key] for instance in instances])
            else:
                ret[key] = [instance[key] for instance in instances]
        return ret