...
class RLTrainer(object):
    ...
    def step_with_rollouts(self, rollouts):
        ...
        for epoch_idx in range(self.args.noptepochs):
            for batch_idx, rollouts_batch in tqdm.tqdm(
                enumerate(rollouts_dataloader, 1), desc="gradstep"
            ):  
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.args.latest_checkpoint_path is not None and not self.args.kl_analysis:
            checkpoint = torch.load(os.path.join(self.args.latest_checkpoint_path, "trainer_state.bin"))
            self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            self.lr_scheduler.load_state_dict(checkpoint['lr_scheduler_dict'])
            self.lr_scheduler.base_lrs = [self.args.learning_rate, self.args.learning_rate]
            self.lr_scheduler.step()
    ...
    def train(self):
        ...
        if self.args.debug:
            ...
            ddtype = torch.bfloat16
        if self.args.latest_checkpoint_path is not None and not self.args.kl_analysis:
            # get last step idx
            latest_step = int(self.args.latest_checkpoint_path.split('-')[-1])
        else:
            latest_step = -1
        with autocast(dtype=ddtype):
            for step_idx in tqdm.tqdm(
                range(FIRST_STEP_IDX, total_steps + FIRST_STEP_IDX),
                desc="steps",
                total=total_steps,
            ):  
                if step_idx != -1 and step_idx <= latest_step:
                    # step dataloader
                    _ = [next(infinite_train_dataloader) for _ in range(self.args.rollout_accumulation_steps)]
                    continue
                ...
                if self.args.grid_search and step_idx == 50:
                    exit()
    ...
    def evaluate(self, step_idx: int, unwrapped_policy=None):
        ...
        # Start evaluation.
        self.policy.eval()
        unwrapped_policy = self.policy.policy.base_model
        
        # for debugging
        if self.args.debug:
            prompts = prompts[:8]
            list_dict_data = list_dict_data[:8]
        ...
        logger.warning(f"Step: {step_idx} => Average reward: {avg_reward}", main_process_only=True)
        wandb.log({"eval/avg_reward": avg_reward}, step=step_idx)
