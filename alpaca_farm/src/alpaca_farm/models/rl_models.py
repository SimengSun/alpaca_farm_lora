...
class AutoregressivePolicy(Policy):
    def forward(
        ...
        if self.args.debug and last_hidden_state.requires_grad:
            last_hidden_state.retain_grad()
            last_hidden_state.register_hook(lambda grad: print("last_hidden_state grad", grad))
        return dict(
...
class Value(nn.Module, abc.ABC):
    def __init__(
        ...
        value_head = torch.nn.Parameter(torch.zeros(1, hidden_size))
        torch.nn.init.zeros_(value_head)
        value_head_bias = torch.nn.Parameter(torch.zeros(1))
        self.value_head = value_head
        self.value_head_bias = value_head_bias
...
class AutoregressiveValue(Value):
    def forward(self, queries: Tensor, query_attn_masks: Tensor, responses: Tensor,
                ) -> Dict[str, Tensor]:
        ...
        values = torch.einsum("btd,d->bt", last_hidden_state, self.value_head.squeeze())
        values = values + self.value_head_bias
        if self.args.debug and values.requires_grad:
            values.retain_grad()
            values.register_hook(lambda grad: print("values grad", grad))
            last_hidden_state.retain_grad()
            last_hidden_state.register_hook(lambda grad: print("value last_hidden_state grad", grad))
        return dict(values=values)
    
    def encode(self, queries: Tensor, query_attn_masks: Tensor) -> Dict[str, Tensor]:
        inputs = self.base_model.prepare_inputs_for_generation(
            input_ids=queries,
            attention_mask=query_attn_masks,
            use_cache=False,
        ...
        outputs = self.base_model.model(**inputs, return_dict=True)
        mask_ = query_attn_masks.ne(0) * torch.arange(queries.size(1), device=queries.device)
        mask_ = mask_.argmax(dim=1)
        last_hidden_state = outputs.last_hidden_state[torch.arange(queries.size(0), device=queries.device), mask_]
        return dict(last_hidden_state=last_hidden_state)
